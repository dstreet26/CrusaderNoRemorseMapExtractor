...
program
    ...
    .action(async (opts) => {
    try {
        const gd = (0, gamedata_1.loadGameData)(opts.inputDataDir);
        ...
        if (opts.rawIndex) {
            ...
            mapIndices = map_1.REMORSE_MISSIONS[levelNum];
            if (!mapIndices) {
                console.error(`Unknown mission ${levelNum}. Valid missions: ${Object.keys(map_1.REMORSE_MISSIONS).join(", ")}`);
                process.exit(1);
            }
        }
        ...
        for (const idx of mapIndices) {
            const mapData = (0, flx_1.getFlxEntryData)(gd.fixedArchive, idx);
            if (!mapData) {
                console.warn(`  Warning: No data for map index ${idx}`);
                continue;
            }
            const fixedItems = (0, map_1.parseFixedItems)(mapData);
            console.log(`  Map ${idx}: ${fixedItems.length} fixed items`);
            const resolved = (0, map_1.resolveMapItems)(fixedItems, gd.globs, gd.typeFlags);
        ...
        const result = await (0, renderer_1.renderMap)(allItems, gd.shapesArchive, gd.palette, {
            bgColor,
            scale: scaleVal,
            floorMinZ,
            floorMaxZ,
...
program
    ...
    try {
        ...
        const shapeData = (0, flx_1.getFlxEntryData)(gd.shapesArchive, archiveIdx);
        if (!shapeData) {
            console.error(`Shape ${shapeNum} not found in SHAPES.FLX`);
            process.exit(1);
        }
        const shape = (0, shape_1.parseShape)(shapeData, gd.palette);
        console.log(`Shape ${shapeNum}: ${shape.frameCount} frames`);
        if (opts.frame !== undefined) {
            const frameNum = parseInt(opts.frame, 10);
            const png = await (0, renderer_1.renderShapeToPng)(gd.shapesArchive, gd.palette, shapeNum, frameNum);
            if (!png) {
            ...
            for (let f = 0; f < shape.frameCount; f++) {
                const png = await (0, renderer_1.renderShapeToPng)(gd.shapesArchive, gd.palette, shapeNum, f);
                if (png) {
...
program
    ...
    try {
        ...
        for (let s = start; s <= end; s++) {
            const shapeData = (0, flx_1.getFlxEntryData)(gd.shapesArchive, s - 1);
            if (!shapeData || shapeData.length < 6)
                continue;
            const shape = (0, shape_1.parseShape)(shapeData, gd.palette);
            for (let f = 0; f < shape.frameCount; f++) {
                ...
                if (frame.width === 0 || frame.height === 0)
                    continue;
                const png = await (0, renderer_1.renderShapeToPng)(gd.shapesArchive, gd.palette, s, f);
                if (png) {
...
program
    ...
    try {
        ...
        for (let i = 0; i < gd.fixedArchive.entryCount; i++) {
            const data = (0, flx_1.getFlxEntryData)(gd.fixedArchive, i);
            if (data && data.length > 0) {
                const itemCount = Math.floor(data.length / 16);
                // Find which mission this maps to
                let mission = "";
                for (const [m, indices] of Object.entries(map_1.REMORSE_MISSIONS)) {
