...
export function loadPalette(filePath: string): Palette {
  ...
  return parsePalette(buf);
}

/**
 * Parse palette from buffer
 */
export function parsePalette(buf: Buffer): Palette {
  ...
  for (let i = 0; i < 256; i++) {
    ...
    const b6 = buf[i * 3 + 2];
    palette.push({
      r: Math.round((r6 * 255) / 63),
      g: Math.round((g6 * 255) / 63),
      b: Math.round((b6 * 255) / 63),
      a: i === 0xff ? 0 : 255, // Index 0xFF is transparent
    });
  }
  return palette;
}
 ...
 */
export function getHardcodedPalette(): Palette {
  const raw6bit: [number, number, number][] = [
